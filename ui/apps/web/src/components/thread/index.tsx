...
export function Thread() {
  ...
  // SDK 的 stream.interrupt 只取 tasks.at(-1)，转人工时最后一个 task 是 output_guard
  // 拿不到 human 节点里的 interrupt，这里自己遍历所有 tasks 找
  const currentInterrupt = (() => {
    if (isLoading) return undefined;
    const latestState = stream.history?.at(-1) as
          ...
          <StickyToBottomContent
            ...
            content={
              <>
                ...
                {hasNoAIOrToolMessages && !!currentInterrupt && (
                  <AssistantMessage
                    key="interrupt-msg"
                    message={undefined}
                    isLoading={isLoading}
                ...
                {/* 兜底：interrupt 发生时最后一条不是 AI 消息（如 ToolMessage 后的转人工），
                    统一在线程末尾渲染 interrupt 视图，避免被 AssistantMessage 的 isLastMessage 条件漏掉 */}
                {!!currentInterrupt &&
                  currentInterrupt.value &&
                  typeof currentInterrupt.value === "object" &&
                  !hasNoAIOrToolMessages &&
                  messages[messages.length - 1]?.type !== "ai" &&
                  (() => {
                    const interruptValue = currentInterrupt
                      .value as Record<string, any>;
                    return isAgentInboxInterruptSchema(interruptValue) ? (
